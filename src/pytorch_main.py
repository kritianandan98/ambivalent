...
# change this to offline for debugging
os.environ["WANDB_MODE"] = "online"


def convert_to_list_str(array2d: np.ndarray):
    result = []
    for row in array2d:
...
def train(args: argparse.Namespace):
    ...
    if resume_epoch:
        resume_checkpoint_path = os.path.join(
            checkpoints_dir, "{}_epoch.pth".format(resume_epoch)
        )
    ...
    while epoch < n_epochs:
        ...
        for batch_id, batch_data_dict in tqdm(
            ...
            batch_data_dict["target"] = move_data_to_device(
                batch_data_dict["target"], device
            )
            batch_target_dict = {"target": batch_data_dict["target"]}

            batch_loss, new_targets = loss_func(
                y_pred=batch_output_dict["clipwise_output"],
                y_true=batch_target_dict["target"],
                cur_time=iteration,
            )
            ...
            if ((batch_id + 1) % grad_accum == 0) or (
                batch_id + 1 == len(train_loader)
        ...
        wandb.log(
            {
                "train labels corrected": labels_corrected,
                "train labels changed": labels_changed,
            },
            step=epoch,
        )
        model.eval()
        ...
        logging.info(
            "Epoch: {} Train Loss  : {:.3f}, ".format(
                epoch, train_statistics["cce-hard"]
            )
        )
        wandb.log(
            {
                "train/loss": train_statistics["cce-hard"],
                "train/f1": train_statistics["f1_score"],
            },
            step=epoch,
        )
        ...
        wandb.log(
            {
                "val/cce-hard": val_statistics["cce-hard"],
                "val/cce-soft": val_statistics["cce-soft"],
                "val/f1": val_statistics["f1_score"],
                "val/ece": val_statistics["ece"],
                "val/brier-hard": val_statistics["brier-hard"],
                "val/brier-soft": val_statistics["brier-soft"],
                "val/kl-hard": val_statistics["kl-hard"],
                "val/kl-soft": val_statistics["kl-soft"],
            },
            step=epoch,
        )
        ...
        if val_f1 > best_valid_f1 and os.environ["WANDB_MODE"] == "online":
            ...
            checkpoint = {"epoch": epoch, "model": model.module.state_dict()}
            #
...
def test(args: argparse.Namespace):
    ...
    # test_hdf5_path = os.path.join(workspace, 'features', feature_name + '_' + 'val_waveform.h5')
    ...
    if "Wav2Vec2" in run_name:
        logging.info(
            "Extracting t-SNE embeddings.. (Change test_len to 500 if this gets stuck)"
        )
        # Load original pre-trained model and processor
        test_len = len(test_loader)
        # test_len = 500
        x = torch.vstack(
            [torch.Tensor(dataset[i]["features"][0]) for i in range(test_len)]
        )
        x = x.to("cuda")
        gt = np.array(
            [config.idx_to_lb[np.argmax(dataset[i]["gt"])] for i in range(test_len)]
        )
        ...
        for input in x:
            ...
            embed = torch.mean(
                model.upstream(input)["last_hidden_state"], dim=1
            ).squeeze()
    ...
    cm = metrics.confusion_matrix(
        np.argmax(targets, axis=-1), np.argmax(predictions, axis=-1), labels=None
    )
