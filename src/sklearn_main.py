...
def upsample(classes: dict, features, targets):
    temp_df = pd.DataFrame(data = np.concatenate([features, targets.reshape(-1, 1)], axis=-1), columns=['f1', 'f2', 'f3', 'f4', 'f5', 'f6', 'f7', 'f8', 'target'])

    for target, num in classes.items():
        sampled_df = temp_df.loc[temp_df['target'] == target].sample(num, random_state=5, replace=True)
...
def get_data(hdf5_path):
    ...
    return np.array(features), np.argmax(np.array(targets, dtype='int'), axis=-1), np.argmax(np.array(gt, dtype='int'), axis=-1)
...
def main(args):
    ...
    test_hdf5_path = os.path.join(workspace, 'features',  feature_name + '_' + 'test_waveform.h5')
    ...
    if model_type == "mlp":
        model = MLPClassifier(hidden_layer_sizes=(650, ), activation='relu', solver='adam', alpha=0.0001,
        ...
        model = RandomForestClassifier(n_estimators=1200, min_samples_split=25, class_weight='balanced', verbose=True)
    elif model_type == 'xgboost':
        model = xgb.XGBClassifier(max_depth=7, learning_rate=0.008, objective='multi:softprob', 
